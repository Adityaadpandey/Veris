# Claim Page + Owner Dashboard Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign `ClaimPage.jsx` and `OwnerDashboard.jsx` to match the approved mockups — split-layout claim page with AI score ring, and a workflow-first dashboard with device status, featured card, and AI score badges.

**Architecture:** Both files are self-contained React components that are rewritten in place. No new files, no new routes. One new backend field (`ai_score`) is needed in the claim API response but the UI degrades gracefully when it's absent.

**Tech Stack:** React 18, Tailwind v4, lucide-react, @privy-io/react-auth, wagmi, axios
...
| File | Change |
|------|--------|
| `owner-portal/src/components/ClaimPage.jsx` | Full rewrite |
| `owner-portal/src/components/OwnerDashboard.jsx` | Full rewrite |

---

## Task 1: Rewrite ClaimPage.jsx

**Files:**
- Modify: `owner-portal/src/components/ClaimPage.jsx`

- [ ] **Step 1: Replace the entire file with the redesigned version**

Write this complete file to `owner-portal/src/components/ClaimPage.jsx`:
...
- [ ] **Step 2: Start the dev server and verify ClaimPage visually**

```bash
cd owner-portal && npm run dev
```

Navigate to `http://localhost:3000/claim/test-123` (any claim ID). Verify:
- Header shows Veris logo + pulsing "Verified Original" green pill
- Split layout renders (photo left, stats right) on desktop
- AI Score ring shows `—` with no score, or a ring if claim has `ai_score`
- Three proof stat cards render (green/orange/blue)
- Accordion toggles open/closed on click
- Footer shows "No trust required, just math."
- On mobile width (<640px), columns stack vertically

Stop the server with Ctrl+C when done.

- [ ] **Step 3: Commit ClaimPage**

```bash
cd owner-portal
git add src/components/ClaimPage.jsx
git commit -m "feat: redesign claim page with split layout and AI score ring"
```

---

## Task 2: Rewrite OwnerDashboard.jsx

**Files:**
- Modify: `owner-portal/src/components/OwnerDashboard.jsx`

- [ ] **Step 1: Replace the entire file with the redesigned version**

Write this complete file to `owner-portal/src/components/OwnerDashboard.jsx`:
...
- [ ] **Step 2: Start the dev server and verify Dashboard visually**
...
- Sidebar: Veris logo, device status card (green pulsing dot when online, grey "Connecting…" when offline), Photos nav with amber pending badge, wallet address + sign out
- Stats row: 4 colored cards (grey/green/blue/amber), emoji icons, "needs retry" sub-label on pending card
- Filter tabs: pill tabs with colored count badges
- Photo grid: featured card spans 2 columns with live pulsing overlay when status is `saved`/`uploaded`; standard cards show AI score badge bottom-right when `img.ai_score` is present
- Standard photo cards: orange QR button, copy link button, Etherscan icon link, Retry button for pending
- Device tab: status dots next to each service badge
...
```bash
cd owner-portal
git add src/components/OwnerDashboard.jsx
git commit -m "feat: redesign owner dashboard with device status and featured card"
```
...
**Spec coverage:**
- ✅ Split claim layout (photo left, stats right, mobile stacks)
- ✅ Animated "Verified Original" pill header
- ✅ AI Score ring with gradient + contextual label
- ✅ Three proof stat cards (green/orange/blue)
- ✅ Proof accordion collapsed by default
- ✅ Claim states: open / pending / minted / not-found
- ✅ "Original · Not AI" overlay on photo
- ✅ "⛓ On-Chain" badge on photo
- ✅ Footer: "No trust required, just math."
- ✅ Device status card in sidebar (pulsing green dot)
- ✅ Nav pending badge (amber pill)
- ✅ Stat cards with icons and color accents
- ✅ Filter tabs with colored counts
- ✅ Featured card (latest shot, 2 columns wide, live pulsing when not minted)
- ✅ AI score badge on photo cards
- ✅ Inline QR / copy / Etherscan actions on photo cards
- ✅ Device tab status dots

**Placeholder scan:** No TBDs. All code complete.

**Type consistency:** `img.ai_score`, `img.claimId`, `img.filecoinCid`, `img.txHash`, `img.createdAt`, `img.status`, `img.id` — all match existing field names in the current `OwnerDashboard.jsx`. `claim.ai_score` is new; the component handles `null`/`undefined` gracefully via `?? null`.
